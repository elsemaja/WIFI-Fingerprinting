###############################################################################
#                                                                             #
#   WIFI | TRAINING RF FOR BUILDINGID | VERSION 7.0 | by ELSE                #
#                                                                             #
...
modelRF <- train(BUILDINGID~., data = setTraining, 
                  method = "rf", 
...
predBUILDING_RF <- predict(modelRF, setTest)


#create a new column with predicted data
setTest$predBUILDING_RF <- predBUILDING_RF
...
setTest$predBUILDING_RF <- as.factor(setTest$predBUILDING_RF)

# check the metrics postResample() for regression and confusionMatrix() for classification ---
confusionMatrix(setTest$predBUILDING_RF, setTest$BUILDINGID)




# make predictions with the model and predict the BUILDING ID of from the validationData ----
predBUILDING_RF <- predict(modelSVM, validationData)

plot(predBUILDING_RF)



#create a new column with predicted data
validationData$predBUILDING_RF <- predBUILDING_RF

# and actual values from that dataset, make sure column of validationData is also factor
validationData$BUILDINGID <- as.factor(validationData$BUILDINGID)
validationData$predBUILDING_RF <- as.factor(validationData$predBUILDING_RF)


# check the metrics postResample() for regression and confusionMatrix() for classification ---
confusionMatrix(validationData$predBUILDING_RF, validationData$BUILDINGID)
...
validationData$predBUILDING_RF <- as.integer(validationData$predBUILDING_RF)
validationData$BUILDINGID <- as.integer(validationData$BUILDINGID)
validationData  <- mutate(validationData, errorsBUILDING = predBUILDING_RF - BUILDINGID) 
...
wrongBUILDING$predBUILDING_RF <- as.factor(wrongBUILDING$predBUILDING_RF)
ggplot(wrongBUILDING, aes(x=LONGITUDE, y=LATITUDE))+
  geom_point() + 
  theme_light() +
...
resultsBUILDING$predBUILDING_RF <- predBUILDING_RF
...
resultsBUILDING$predBUILDING_RF <- as.integer(resultsBUILDING$predBUILDING_RF)
resultsBUILDING$BUILDINGID <- as.integer(resultsBUILDING$BUILDINGID)

resultsBUILDING  <- mutate(resultsBUILDING, errorsBUILDING = predBUILDING_RF - BUILDINGID) 
resultsBUILDING$errorsBUILDING <- resultsBUILDING$predBUILDING_RF - resultsBUILDING$BUILDINGID
