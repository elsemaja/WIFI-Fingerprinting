###############################################################################
#                                                                             #
#   WIFI | VISUALISING THE RESULTS WITH PLOTS | VERSION 2.0 | by ELSE         #
#                                                                             #
...
resultsVSactual <- readRDS(file = "resultsVSactual(2).rds")
...
PlotPred3D <- plot_ly(resultsVSactual, 
                  x = ~predLATITUDE_KNN, 
                  y = ~predLONGITUDE_KNN, 
                  z = ~PredictedFLOOR_KNN, 
                  color = ~PredictedBUIDLING_SVM, 
...
PlotError3D <- plot_ly(resultsVSactual, 
                      x = ~LATITUDE, y = ~LONGITUDE, z = ~FLOOR, colors = c('#BF382A', '#0C4B8E')) %>%
