###############################################################################
#                                                                             #
#   WIFI | TRAINING RF MODEL FOR LATITUDE | FINAL VERSION  | by ELSE          #
#                                                                             #
...
modelRF <- train(LATITUDE~., data = setTraining, method = "rf", trControl = CrossValidation)
...
predLATITUDE_RF <- predict(modelRF, setTest)
...
setTest$predLATITUDE_RF <- predLATITUDE_RF
...
predLATITUDE_RF <- predict(modelRF, validationData)


#create a new column with predicted data
validationData$predLATITUDE_RF <- predLATITUDE_RF


# check the metrics postResample() for regression and confusionMatrix() for classification ---
postResample(validationData$predLATITUDE_RF, validationData$LATITUDE)
...
validationData$predLATITUDE_RF <- as.numeric(validationData$predLATITUDE_RF)


# add column with errors to the dataframe
validationData  <- mutate(validationData, errorsLATITUDE = predLATITUDE_RF - LATITUDE) 
...
resultsLATITUDE$predLATITUDE_RF <- predLATITUDE_RF
resultsLATITUDE$LATITUDE <- validationData$LATITUDE


# mutate the errors and add them to the tibble
resultsLATITUDE  <- mutate(resultsLATITUDE, errorsLATITUDE = predLATITUDE_RF - LATITUDE) 
resultsLATITUDE$errorsLATITUDE <- resultsLATITUDE$predLATITUDE_RF - resultsLATITUDE$LATITUDE
